...
public class UserRoleServiceImpl implements IUserRoleService {
    ...
    private final IUserRoleBatchService userRoleBatchService;

    public UserRoleServiceImpl(UserRoleMapper userRoleMapper, UserMapper userMapper, IUserRoleBatchService userRoleBatchService) {
        this.userRoleMapper = userRoleMapper;
        this.userMapper = userMapper;
        this.userRoleBatchService = userRoleBatchService;
    }
    ...
    public void bindUserRoles(Long userId, List<Long> roleIds, Long parkId) {
        if (userId == null) {
            throw new BizException(BizCodes.BAD_REQUEST, "绑定用户角色时 userId 不能为空");
        }
        if (parkId == null) {
            throw new BizException(BizCodes.BAD_REQUEST, "绑定用户角色时必须选择园区；各园区独立保存，互不影响其他园区已有绑定");
        }
        if (roleIds == null || roleIds.isEmpty()) {
            throw new BizException(BizCodes.BAD_REQUEST, "绑定用户角色时 roleIds 不能为空");
        }
        User user = userMapper.selectById(userId);
        if (user != null && "admin".equalsIgnoreCase(user.getUsername())) {
            throw new BizException(BizCodes.FORBIDDEN, "admin 账号为系统保留账号，禁止在此页面绑定角色");
        }
        ...
        if (roleIds != null && !roleIds.isEmpty()) {
            ...
            userRoleBatchService.saveBatch(batch);
        }
